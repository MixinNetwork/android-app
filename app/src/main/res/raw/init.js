(function() {
    var config = {
        ethereum: {
            chainId: "%1$s",
            rpcUrl: "%2$s",
            address: "%3$s",
            overwriteMetamask: true,
        },
        solana: {
            cluster: "%4$s",
        },
        tron: {
            nodeURL: "%5$s",
        },
        isDebug: false
    };
    const core = mixinwallet.core('CALLBACK', (params) => {
        window._mw_.postMessage(JSON.stringify(params));
    });
    const ethereum = mixinwallet.ethereum(config.ethereum);
    const solana = mixinwallet.solana(config.solana);
    const tron = mixinwallet.tron(config.tron);

    core.registerProviders([ethereum, solana, tron]);

    ethereum.emitChainChanged = (chainId) => {
        const normalizedChainId = '0x' + parseInt(chainId || '1').toString(16);
        ethereum.setChainId(normalizedChainId);
        ethereum.emit('chainChanged', normalizedChainId);
        ethereum.emit('networkChanged', parseInt(normalizedChainId).toString(10));
    };

    ethereum.setConfig = (nextConfig) => {
        const ethereumConfig = nextConfig.ethereum || nextConfig;
        ethereum.setChainId('0x' + parseInt(ethereumConfig.chainId || '1').toString(16));
        if (ethereumConfig.address) {
            ethereum.setAddress(ethereumConfig.address);
        }
        if (ethereumConfig.rpcUrl) {
            ethereum.setRPCUrl(ethereumConfig.rpcUrl);
        }
    };

    Object.defineProperties(mixinwallet, {
        ethereum: {
            configurable: false,
            enumerable: true,
            writable: false,
            value: ethereum,
        },
        solana: {
            configurable: false,
            enumerable: true,
            writable: false,
            value: solana,
        },
        tron: {
            configurable: false,
            enumerable: true,
            writable: false,
            value: tron,
        },
    });

    window.ethereum = ethereum;
    window.solana = solana;
    window.tronLink = tron;
    window.tronWeb = tron.tronWeb;

    const mixinLogoDataUrl = 'data:image/svg+xml;base64,';
    const info = {
        uuid: mixinwallet.randomUUID(),
        name: 'Mixin Messenger',
        icon: mixinLogoDataUrl,
        rdns: 'one.mixin.messenger',
    };

    ethereum.setAddress(config.ethereum.address);

    function initializeEIP6963(provider, options = {}) {
        const providerDetail = {
            info,
            provider
        };
        Object.defineProperty(providerDetail, 'provider', {
            get() {
                options.onAccessProvider?.();
                return provider;
            },
        });
        const announceEvent = new CustomEvent('eip6963:announceProvider', {
            detail: Object.freeze(providerDetail),
        });
        window.dispatchEvent(announceEvent);
        window.addEventListener('eip6963:requestProvider', () => {
            window.dispatchEvent(announceEvent);
            options.onRequestProvider?.();
        });
    }
    initializeEIP6963(ethereum);
})();
