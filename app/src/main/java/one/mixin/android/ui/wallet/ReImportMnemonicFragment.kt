...
class ReImportMnemonicFragment : BaseFragment(R.layout.fragment_compose) {
            ...
            MnemonicPhraseInput(
                ...
                onComplete = { words ->
                    lifecycleScope.launch {
                        ...
                        ensureTronAddress(walletId, words)
    ...
    private fun resolveDerivationIndex(): Int? {
        val candidatePath: String? = evmAddressInfo?.path ?: solAddressInfo?.path ?: btcAddressInfo?.path ?: tronAddressInfo?.path
    ...
    private suspend fun ensureTronAddress(walletId: String, mnemonic: List<String>) {
        if (viewModel.getAddressesByChainId(walletId, Constants.ChainId.TRON_CHAIN_ID) != null) {
            return
        }
        val addresses = listOf(
            viewModel.getAddressesByChainId(walletId, Constants.ChainId.ETHEREUM_CHAIN_ID),
            viewModel.getAddressesByChainId(walletId, Constants.ChainId.SOLANA_CHAIN_ID),
            viewModel.getAddressesByChainId(walletId, Constants.ChainId.BITCOIN_CHAIN_ID),
        )
        val candidatePath = addresses.firstNotNullOfOrNull { it?.path }
        val derivationIndex = requireNotNull(candidatePath?.let(CryptoWalletHelper::extractIndexFromPath))
        val derivedWallet = CryptoWalletHelper.mnemonicToTronWallet(mnemonic.joinToString(" "), index = derivationIndex)
        ...
        if (!response.isSuccess) {
            Timber.e(
                "Failed to update Tron address walletId=$walletId errorCode=${response.errorCode} errorDescription=${response.errorDescription}"
            )
        } else {
            jobManager.addJobInBackground(RefreshSingleWalletJob(response.data!!.id))
        }
            ...
            if (wallet.category == WalletCategory.IMPORTED_MNEMONIC.value) {
                ...
                val isWalletMatch: Boolean =
                    (evmAddress == null || isEvmMatch) &&
                        ...
                        (isEvmMatch || isSolanaMatch || isTronMatch)
                if (!isWalletMatch) {
                    return@forEach
                }
                ...
                runCatching { ensureTronAddress(wallet.id, mnemonic) }
