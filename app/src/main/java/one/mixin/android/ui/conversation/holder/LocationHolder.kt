...
class LocationHolder constructor(val binding: ItemChatLocationBinding) : BaseViewHolder(binding.root), OnMapReadyCallback {
    ...
    init {
        ...
        mixinMapView = MixinMapView(binding.googleMap, binding.amap)
        mixinMapView.onCreate(null)
        if (useGoogleMap) {
            binding.googleMap.getMapAsync(this)
        } else {
            initAMap(binding.amap.map)
        }
    }
    ...
    private fun initAMap(aMap: AMap) {
        com.amap.api.maps.MapsInitializer.initialize(MixinApplication.appContext)
    ...
    private fun setMapLocation() {
        if (useGoogleMap) {
            ...
            if (mixinMapView.aMap == null) return
        }
