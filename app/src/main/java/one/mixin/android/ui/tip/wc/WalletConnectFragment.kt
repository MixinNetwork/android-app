...
import one.mixin.android.ui.tip.wc.connection_details.ConnectionDetailsPage
...
private class WCNavControllerImpl(private val navController: NavController): WCNavController() {
    override fun navTo(destination: WCDestination, args: Bundle?) {
        val routeLink = NavDeepLinkRequest.Builder
            .fromUri(NavDestination.createRoute(destination.name).toUri())
            .build()
        val deepLinkMatch = navController.graph.matchDeepLink(routeLink)
        if (deepLinkMatch == null) {
            Timber.w("navigateTo: no match for $destination")
            return
        }
