...
class TransferDestinationInputFragment() : BaseFragment(R.layout.fragment_address_input) {
    ...
    private suspend fun handleWeb3ExternalTransfer(url: String) {
        ...
        val c = web3ViewModel.findAndRefreshWeb3TokenItem(walletId, t.chainId)
        if (c == null) {
            Timber.e("[$TAG] handleWeb3ExternalTransfer web3 chain token not found for chainId=%s", t.chainId)
            toast(R.string.Data_error)
            return
        }
        val from = web3ViewModel.getAddressesByChainId(walletId = walletId, c.chainId)?.destination
        if (from == null) {
