...
class PerpsPositionShareBottomFragment : MixinBottomSheetDialogFragment() {
    ...
    private fun updatePosterPagerHeight() {
        ...
        val width = binding.posterPager.width.takeIf { it > 0 } ?: return
        itemView.measure(
            View.MeasureSpec.makeMeasureSpec(width, View.MeasureSpec.EXACTLY),
            View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED),
        )
        val measuredHeight = itemView.measuredHeight.takeIf { it > 0 } ?: return
        if (binding.posterPager.layoutParams.height == measuredHeight && binding.posterContainer.layoutParams.height == measuredHeight) return
