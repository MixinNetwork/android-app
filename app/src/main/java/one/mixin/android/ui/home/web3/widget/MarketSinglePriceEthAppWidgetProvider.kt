...
class MarketSinglePriceEthAppWidgetProvider : AppWidgetProvider() {
    ...
    private fun buildRemoteViews(context: Context, appWidgetId: Int): RemoteViews {
        val views: RemoteViews = MarketSinglePriceWidgetRenderer.buildRemoteViews(
            context = context,
            coinId = COIN_ID,
            title = "ETH ${context.getString(R.string.Price)}",
        )
        return views
    }
    ...
    companion object {
        private const val COIN_ID: String = "ethereum"
        private const val UNIQUE_ONE_TIME_WORK_NAME: String = "MarketSinglePriceWidgetWorker.ETH.Once"
        private const val UNIQUE_PERIODIC_WORK_NAME: String = "MarketSinglePriceWidgetWorker.ETH"

        fun notifyWidgetDataChanged(context: Context) {
            val appWidgetManager: AppWidgetManager = AppWidgetManager.getInstance(context)
            val componentName = ComponentName(context, MarketSinglePriceEthAppWidgetProvider::class.java)
            val appWidgetIds: IntArray = appWidgetManager.getAppWidgetIds(componentName)
            appWidgetIds.forEach { appWidgetId ->
                val views: RemoteViews = MarketSinglePriceWidgetRenderer.buildRemoteViews(context, COIN_ID, "ETH Price")
                appWidgetManager.updateAppWidget(appWidgetId, views)
            }
        }
    }
