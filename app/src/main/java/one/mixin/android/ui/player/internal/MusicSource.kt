...
import one.mixin.android.extension.containsCaseInsensitive
...
/**
 * Base class for music sources in Mixin.
 */
    ...
    override fun search(query: String, extras: Bundle): List<MediaMetadataCompat> {
        ...
        val focusSearchResult = when (extras[MediaStore.EXTRA_MEDIA_FOCUS]) {
            MediaStore.Audio.Genres.ENTRY_CONTENT_TYPE -> {
                ...
                Timber.d("$TAG Focused genre search: '$genre'")
                filter { song ->
                    song.genre == genre
                }
            }
            MediaStore.Audio.Artists.ENTRY_CONTENT_TYPE -> {
                ...
                Timber.d("$TAG Focused artist search: '$artist'")
                filter { song ->
            ...
            MediaStore.Audio.Albums.ENTRY_CONTENT_TYPE -> {
                ...
                Timber.d("$TAG Focused album search: album='$album' artist='$artist")
                filter { song ->
            ...
            MediaStore.Audio.Media.ENTRY_CONTENT_TYPE -> {
                ...
                val album = extras[MediaStore.EXTRA_MEDIA_ALBUM]
                val artist = extras[MediaStore.EXTRA_MEDIA_ARTIST]
                Timber.d("$TAG Focused media search: title='$title' album='$album' artist='$artist")
                filter { song ->
                    (song.artist == artist || song.albumArtist == artist) && song.album == album
                            && song.title == title
                }
        ...
        if (focusSearchResult.isEmpty()) {
            return if (query.isNotBlank()) {
                Timber.d("$TAG Unfocused search for '$query'")
                filter { song ->
                ...
                Timber.d("$TAG Unfocused search without keyword")
                return shuffled()
            }
        } else {
