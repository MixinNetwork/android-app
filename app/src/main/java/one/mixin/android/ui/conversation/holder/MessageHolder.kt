...
class MessageHolder constructor(containerView: View) : BaseViewHolder(containerView) {
    ...
    init {
        ...
        itemView.chat_tv.addAutoLinkMode(AutoLinkMode.MODE_TRANSFER)
        ...
        itemView.chat_tv.setTransferModeColor(LINK_COLOR)
        ...
        itemView.chat_tv.setAutoLinkOnClickListener { autoLinkMode, matchedText ->
            when (autoLinkMode) {
                ...
                AutoLinkMode.MODE_TRANSFER -> {
                    onItemListener?.onTransferClick(matchedText.substring(matchedText.lastIndexOf('/') + 1))
                }
                else -> {
                }
            }
