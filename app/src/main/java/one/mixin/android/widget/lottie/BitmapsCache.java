...
public class BitmapsCache {
    ...
    private Runnable cleanupSharedBuffers = new Runnable() {
        ...
        public void run() {
            ...
            if (!sharedBuffers.isEmpty()) {
                uiHandler.postDelayed(cleanupSharedBuffers, 5000);
            } else {
    ...
    public BitmapsCache(File sourceFile, Cacheable source, CacheOptions options, int w, int h, boolean noLimit) {
        ...
        File fileTmo = new File(MixinApplication.appContext.getCacheDir(), "acache");
        if (!mkdir) {
            fileTmo.mkdir();
            mkdir = true;
        }
        ...
        if (Utilities.getDevicePerformanceClass() >= Utilities.PERFORMANCE_CLASS_HIGH) {
        ...
        try {
            ...
            while (true) {
                ...
                if (cancelled.get() || closed.get()) {
                    Timber.d("cancelled cache generation");
                    closed.set(true);
                    for (int i = 0; i < N; i++) {
...
//        cancelled.set(true);
    ...
    private byte[] getBuffer(FrameOffset selectedFrame) {
        boolean useSharedBuffers = this.useSharedBuffers && Thread.currentThread().getName().startsWith("DispatchQueuePoolThreadSafety_");
        byte[] bufferTmp;
        if (useSharedBuffers) {
