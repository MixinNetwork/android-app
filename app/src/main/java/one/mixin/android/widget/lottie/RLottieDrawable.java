...
public class RLottieDrawable extends BitmapDrawable implements Animatable, BitmapsCache.Cacheable {
    ...
    private ArrayList<View> parentViews = new ArrayList<>();
    ...
    private RectF[] dstRectBackground = new RectF[2];
    private Paint[] backgroundPaint = new Paint[2];
    ...
    protected void recycleResources() {
        ...
        Utilities.recycleBitmaps(bitmapToRecycle);
    ...
    public boolean setBaseDice(File path) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            ContextExtensionKt.runOnUiThread(() -> {
    ...
    public static String readRes(File path, int rawRes) {
        ...
        try {
            if (path != null) {
                ...
                inputStream = MixinApplication.appContext.getResources().openRawResource(rawRes);
            }
            int readLen;
    ...
    public void addParentView(View parent) {
        if (parent == null) {
            return;
        }
    ...
    public void removeParentView(View parent) {
        if (parent == null) {
            return;
        }
    ...
    protected boolean scheduleNextGetFrame() {
        ...
        if (shouldLimitFps && Thread.currentThread() == Looper.getMainLooper().getThread()) {
    ...
    private void setCurrentFrame(long now, long timeDiff, long timeCheck, boolean force) {
        ...
        singleFrameDecoded = true;
        waitingForNextTask = false;
        if (AndroidUtilities.INSTANCE.getScreenRefreshRate() <= 60) {
            lastFrameTime = now;
        } else {
    ...
    public void updateCurrentFrame(long time, boolean updateInBackground) {
        ...
        } else if (AndroidUtilities.INSTANCE.getScreenRefreshRate() <= 60 || (updateInBackground && AndroidUtilities.INSTANCE.getScreenRefreshRate() <= 80)) {
    ...
    public void drawFrame(Canvas canvas, int frame) {
        ...
        AndroidUtilities.INSTANCE.getRectTmp2().set(0, 0, width, height);
        canvas.drawBitmap(backgroundBitmap, AndroidUtilities.INSTANCE.getRectTmp2(), getBounds(), getPaint());
    }
    ...
    public void checkCache(Runnable onReady) {
        if (bitmapsCache == null) {
            ContextExtensionKt.runOnUiThread(onReady, 0);
            return;
        }
