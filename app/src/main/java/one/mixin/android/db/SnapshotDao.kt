...
interface SnapshotDao : BaseDao<Snapshot> {
    ...
    @Query("SELECT s.*, u.full_name AS opponentFullName, a.symbol AS asset_symbol FROM snapshots s LEFT JOIN users u ON u.user_id = s.opponent_id LEFT JOIN " +
        "assets a ON a.asset_id = s.asset_id WHERE s.asset_id = :assetId AND s.type = :type ORDER BY s.created_at DESC, s.snapshot_id DESC")
    fun snapshotsByType(assetId: String, type: String): LiveData<List<SnapshotItem>>
