...
interface ParticipantSessionDao : BaseDao<ParticipantSession> {
    ...
    @Query("SELECT public_key FROM participant_session WHERE conversation_id = :conversationId AND user_id = :userId LIMIT 1")
    suspend fun findBotPublicKey(conversationId: String, userId: String): String?

    @SuppressWarnings(RoomWarnings.CURSOR_MISMATCH)
    @Query("SELECT * FROM participant_session WHERE conversation_id = :conversationId AND user_id = :userId AND session_id = :sessionId AND public_key IS NOT NULL LIMIT 1")
