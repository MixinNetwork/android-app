...
class MixinDatabaseMigrations private constructor() {
    ...
    companion object {
        ...
        val MIGRATION_50_51: Migration = object : Migration(50, 51) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS `outputs` (`output_id` TEXT NOT NULL, `transaction_hash` TEXT NOT NULL, `output_index` INTEGER NOT NULL, `kernel_asset_id` TEXT NOT NULL, `amount` TEXT NOT NULL, `mask` TEXT NOT NULL, `keys` TEXT NOT NULL, `threshold` INTEGER NOT NULL, `extra` TEXT NOT NULL, `state` TEXT NOT NULL, `created_at` TEXT NOT NULL, `updated_at` TEXT NOT NULL, `signed_by` TEXT NOT NULL, `signed_at` TEXT NOT NULL, `spent_at` TEXT NOT NULL, PRIMARY KEY(`output_id`))")
                database.execSQL("CREATE TABLE IF NOT EXISTS `tokens` (`asset_id` TEXT NOT NULL, `symbol` TEXT NOT NULL, `name` TEXT NOT NULL, `icon_url` TEXT NOT NULL, `price_btc` TEXT NOT NULL, `price_usd` TEXT NOT NULL, `chain_id` TEXT NOT NULL, `change_usd` TEXT NOT NULL, `change_btc` TEXT NOT NULL, `confirmations` INTEGER NOT NULL, `asset_key` TEXT, `reserve` TEXT, `withdrawal_memo_possibility` TEXT, PRIMARY KEY(`asset_id`))")
                ...
                database.execSQL("CREATE TABLE IF NOT EXISTS `raw_transactions` (`request_id` TEXT NOT NULL, `raw_transaction` TEXT NOT NULL, `created_at` TEXT NOT NULL, PRIMARY KEY(`request_id`))")
                database.execSQL("ALTER TABLE `chains` ADD COLUMN `withdrawal_memo_possibility` TEXT DEFAULT 'possible'")
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_outputs_kernel_asset_id_state_created_at` ON `outputs` (`kernel_asset_id`, `state`, `created_at`)")
