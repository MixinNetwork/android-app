...
interface ParticipantDao : BaseDao<Participant> {
    ...
    @Query("SELECT p.conversation_id FROM participants p, conversations c WHERE user_id = :userId AND p.conversation_id = c.conversation_id AND c.status == 2 LIMIT 1")
