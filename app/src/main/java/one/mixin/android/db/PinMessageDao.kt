...
interface PinMessageDao : BaseDao<PinMessage> {
    ...
    @Query("SELECT count(1) FROM pin_messages WHERE created_at < (SELECT created_at FROM pin_messages WHERE conversation_id = :conversationId AND message_id = :messageId) AND conversation_id = :conversationId")
