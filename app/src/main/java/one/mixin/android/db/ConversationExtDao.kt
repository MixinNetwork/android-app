...
interface ConversationExtDao : BaseDao<ConversationExt> {
    ...
    @Query("INSERT INTO conversation_ext (`conversation_id`, `created_at`, `count`) VALUES (:conversationId, :createdAt, (SELECT count(1) FROM messages m INNER JOIN users u ON m.user_id = u.user_id WHERE conversation_id = :conversationId))")
