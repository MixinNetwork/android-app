...
interface PendingMessageDao : BaseDao<PendingMessage> {
    ...
    @Query("SELECT message_id FROM pending_messages WHERE message_id = :messageId")
    fun findMessageIdById(messageId: String): String?

    @Query("SELECT message_id FROM pending_messages WHERE conversation_id = :conversationId AND user_id = :userId AND status = 'FAILED' ORDER BY created_at DESC LIMIT 1000")
    fun findFailedMessages(conversationId: String, userId: String): List<String>

    @Query("SELECT count(message_id) FROM pending_messages WHERE conversation_id = :conversationId AND quote_message_id = :messageId AND quote_content IS NULL")
    fun countMessageByQuoteId(conversationId: String, messageId: String): Int

    @Query(
        """
        SELECT m.message_id FROM pending_messages m
    ...
    @Query("SELECT category, message_id, conversation_id, media_url FROM pending_messages WHERE message_id = :messageId")
    ...
    @Query("UPDATE pending_messages SET content = :content, status = :status WHERE message_id = :id AND category != 'MESSAGE_RECALL'")
    ...
    @Query("UPDATE pending_messages SET sticker_id = :stickerId, status = :status WHERE message_id = :messageId AND category != 'MESSAGE_RECALL'")
    fun updateStickerMessage(stickerId: String, status: String, messageId: String)

    @Query("UPDATE pending_messages SET shared_user_id = :sharedUserId, status = :status WHERE message_id = :messageId AND category != 'MESSAGE_RECALL'")
    ...
    @Query("UPDATE pending_messages SET status = 'SENT' WHERE message_id = :id AND status = 'FAILED'")
    ...
    @Query("DELETE FROM pending_messages WHERE message_id = :id")
    fun deleteById(id: String)

    @Query("DELETE FROM pending_messages WHERE message_id IN (:ids)")
    fun deleteByIds(ids: List<String>)

    @Query("UPDATE pending_messages SET status = 'READ' WHERE message_id IN (:ids)")
