...
interface AssetDao : BaseDao<Asset> {
    companion object {
        ...
        const val POSTFIX_ASSET_ITEM_NOT_HIDDEN = " WHERE ae.hidden ISNULL OR NOT ae.hidden$POSTFIX_ASSET_ITEM"
    }
    ...
    @SuppressWarnings(RoomWarnings.CURSOR_MISMATCH)
    @Query("$PREFIX_ASSET_ITEM WHERE a1.balance > 0 AND (a1.symbol LIKE :symbol" + ESCAPE_SUFFIX + " OR a1.name LIKE :name" + ESCAPE_SUFFIX + ") " +
