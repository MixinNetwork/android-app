...
public abstract class NoCountLimitOffsetDataSource<T> extends PositionalDataSource<T> {
    private final RoomQuery mSourceQuery;
    private final int mTotalCount;
    private final RoomDatabase mDb;

    protected NoCountLimitOffsetDataSource(RoomDatabase db, RoomQuery query, int count, String... tables) {
        mDb = db;
        mTotalCount = count;
        mSourceQuery = query;
    ...
    public List<T> loadRange(int startPosition, int loadCount) {
        final RoomQuery sqLiteQuery = mSourceQuery.withLimitOffset(loadCount, startPosition);
        Cursor cursor = RoomDatabaseCompat.query(mDb, sqLiteQuery);
