...
class TipNode
    @Inject
    internal constructor(private val tipNodeService: TipNodeService, private val tipConfig: TipConfig, private val gson: Gson) {
        ...
        suspend fun sign(
            ...
            val userSk = Tip.newPrivateKeyFromBytes(identityPriv)

            var assigneeSk: ByteArray? = null
            ...
            val pair =
                if (!failedSigners.isNullOrEmpty() && failedSigners.size != tipConfig.signers.size && assigneeSk != null) {
                    // should sign successful signers before failed signers,
                    // prevent signing failed signers with different identities.
                    val successfulSigners = tipConfig.signers - failedSigners.toSet()
            ...
            val maxCounter =
                requireNotNull(data.maxByOrNull { it.counter }?.counter) {
        ...
        suspend fun watch(
            watcher: ByteArray,
            callback: Callback? = null,
        ): Pair<List<TipNodeCounter>, String> {
        ...
        private suspend fun getNodeSigs(
            userSk: ByteArray,
            tipSigners: List<TipSigner>,
            ephemeral: ByteArray,
            watcher: ByteArray,
            assignee: ByteArray?,
            callback: Callback?,
        ): Pair<List<TipSignRespData>, TipNodeError?> {
        ...
        private suspend fun watchTipNode(
            tipSigner: TipSigner,
            watcher: ByteArray,
        ): Pair<Int, Int> {
        ...
        private suspend fun signTipNode(
            userSk: ByteArray,
            tipSigner: TipSigner,
            ephemeral: ByteArray,
            watcher: ByteArray,
        ...
        private fun genTipSignRequest(
            ...
            val sig = Tip.signFromBytes(userSk, msg).toHex()

            val userPkStr = Tip.pointPublicKeyString(userPk)
