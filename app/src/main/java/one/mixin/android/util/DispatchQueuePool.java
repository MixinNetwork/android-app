...
public class DispatchQueuePool {
    ...
    private Runnable cleanupRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (!queues.isEmpty() || !busyQueues.isEmpty()) {
                ContextExtensionKt.runOnUiThread(this, 30000);
    ...
    public void execute(Runnable runnable) {
        ...
        if (!busyQueues.isEmpty() && (totalTasksCount / 2 <= busyQueues.size() || queues.isEmpty() && createdCount >= maxCount)) {
            ...
            queue = new DispatchQueue("DispatchQueuePool" + guid + "_" + CryptoUtilKt.getSecureRandom().nextInt());
        ...
        if (!cleanupScheduled) {
            ContextExtensionKt.runOnUiThread(cleanupRunnable, 30000);
            cleanupScheduled = true;
        }
        totalTasksCount++;
