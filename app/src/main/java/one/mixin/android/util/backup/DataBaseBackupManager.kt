...
class DataBaseBackupManager private constructor(driveResourceClient: DriveResourceClient,
    ...
    fun restoreDatabase(callback: (Result) -> Unit) {
        ...
        if (metaData != null) {
            val file = getDbFile() ?: return callback(NOT_FOUND)
            file.deleteOnExit()
    ...
    private fun deleteAllDb(dbName: String) {
    ...
    private suspend fun uploadDatabase(context: CoroutineContext, driveId: DriveId, file: File, version: Int? = null,
        callback: (Result) -> Unit): DriveFile? {
        val d = GlobalScope.async(context) {
            val uploadTask = uploadBackup(driveId, file, version)
