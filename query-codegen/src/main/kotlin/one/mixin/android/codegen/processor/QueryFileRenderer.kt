...
class QueryFileRenderer {
    ...
    private fun renderPagingSourceFunction(
        ...
        lines += ""
        lines += "            private val invalidation = RoomDatabaseCompat.observeInvalidation(${function.databaseParameter}, this, ${function.tables.joinToString { "\"$it\"" }})"
        lines += ""
        lines += "            override suspend fun load(params: LoadParams<Int>): LoadResult<Int, $itemType> {"
        lines += "                invalidation.awaitStart()"
        lines += "                return withContext(RoomDatabaseCompat.queryContext(${function.databaseParameter})) {"
        ...
        lines += "                        }"
        lines += "                    } else {"
        lines += "                        val loadResult = ${function.databaseParameter}.useReaderConnection { connection ->"
        lines += "                            queryData(connection, params, tempCount)"
        lines += "                        }"
        lines += "                        invalidation.refresh()"
